...
# 실행: docker build -f 05.Streamlit/05-7.Dockerfile -t curriculum-backend .
...
# 05-6.Main.py는 gunicorn 모듈명 호환을 위해 main.py로 복사
COPY 05.Streamlit/05-2.Schemas.py      ./05-2.Schemas.py
COPY 05.Streamlit/05-3.Auth.py         ./05-3.Auth.py
COPY 05.Streamlit/05-4.Indexing.py     ./05-4.Indexing.py
COPY 05.Streamlit/05-5.Retrieval.py    ./05-5.Retrieval.py
COPY 05.Streamlit/05-6.Main.py         ./main.py

# PDF 및 캐시 파일 복사
COPY Data/ ./Data/

# vectorDB는 볼륨으로 마운트 (05-8.docker-compose.yml 참고)
